...
with_v_thermal_init = .true.
with_push =           .true.
with_coll =           .true.
with_thermal_force =  .true.
with_JOREK_proj =     .false.

fix_ne_Te =           .false.

with_E_field =        .false. ! E-field is not physical if simulation is static and was not ran with v_par since the beginning
v_par_init =          0.d0    ! proxy background parallel D flow (compare with experiments and sheath boundary conditions)
...
if (restart_particles) then
  ...
  if (with_psi_n_init .eq. .true.) then
    ...
    call initiliase_particles_as_gaussian_blob(sim, R_=2.07d0, Z_=0.d0, phi_=0.d0, & !R=1.925d0, Z=0.12d0, phi=0.d0, &!
    ...
    do j=1, size(sim%groups(i)%particles) !< particles
      ...
      do k=1, my_nstep_particles !< steps
        ...
        type is (particle_kinetic_leapfrog)
          ...
          limits_coll = T_e_raw * K_BOLTZ / EL_CHG .lt. 1.d0 !< limits for collisions
          !write(*,*) "limits, limits_coll", limits, limits_coll
          !write(*,*) "n_e_raw(m^-3), T_e_raw(eV)", n_e_raw, T_e_raw * K_BOLTZ / EL_CHG, pa%x

          !write(*,*) "ne(m^-3), Te(eV)"
          !write(*,*) "", n_e, T_e * K_BOLTZ / EL_CHG  ! m^-3, eV
          !write(*,*) "gradTe(3)"
          !write(*,*) "", grad_T_e * K_BOLTZ / EL_CHG  ! eV/m
          if (fix_ne_Te .eq. .true.) then
            !n_e = 1. / central_density !1.d19 / (central_density * 1.d20)
            T_e = 20 * Temp_norm ! eV (because of normalization)
            grad_T_e = 0 ! if T is fixed
          end if
