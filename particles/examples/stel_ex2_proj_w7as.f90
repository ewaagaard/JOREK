!> Coronal equilibrium + JOREK projection test for W7-AS geometry.
!> Derived from stel_ex2_proj.f90 (W7-A); only change: blob center R=1.934 m (W7-AS axis).
!> No Boris push, no Coulomb collisions. Static JOREK field from restart H5.
!>
!> What it does:
!>   1. Place N neutral W particles in a Gaussian blob at R=1.934 m (W7-AS magnetic axis)
!>      fix_ne_Te=.true. -> T_e~43 eV so ADAS limits are satisfied
...
!>   Should give <Z> ~ 8 (ADAS coronal at 43 eV, W8+ dominant)
!>   Purpose: validate coupled projection in W7-AS 3D geometry (n_tor=21, n_period=5, n_plane=128)
program stel_ex2_proj_w7as
use data_structure
...
! Gaussian blob at W7-AS magnetic axis (R_axis~1.934 m, sigma=5mm, all at phi=0)
...
end program stel_ex2_proj_w7as
